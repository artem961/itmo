...
public class Main {
    public static void main(String... args) throws IOException, NetworkException, ValidationException {
       ...
       // controller.run();
        ...
        try {
            for (Flat flat: collectionManager.getAsList()){
                System.out.println(new DBQueryManager().insertFlat(flat));
            }
            ...
            while (result.next()){
                for (int i = 1; i <= columnCount; i++) {
                    System.out.print(result.getString(i) + "\t");  // Вывод как строку (универсально)
                }
                System.out.println();
            }
