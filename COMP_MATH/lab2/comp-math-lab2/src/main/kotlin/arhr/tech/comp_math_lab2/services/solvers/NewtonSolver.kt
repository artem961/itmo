...
class NewtonSolver(
    ...
    override fun solve(request: SolveEquationRequest): SolveEquationResponse {
        ...
        do {
            val fx = equation.f(x)
            val dfx = equation.df(x)

            if (dfx.signum() == 0) throw RuntimeException("Производная равна нулю, метод Ньютона не применим")

            // nextX = x - f(x) / f'(x)
            val nextX = x.subtract(fx.divide(dfx, mc))
