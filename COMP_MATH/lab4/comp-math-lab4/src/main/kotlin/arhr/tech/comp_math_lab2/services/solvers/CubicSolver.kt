...
@Component
class CubicSolver : AbstractApproximationSolver(
    ApproximationType.CUBIC
) {
    override fun buildMatrix(points: List<PointDto>): Array<Array<BigDecimal>> {
        ...
        val sumX5 = points.sumOf { it.x.pow(5, mc)}
        val sumX6 = points.sumOf { it.x.pow(6, mc)}

        return arrayOf(
            arrayOf(n, sumX, sumX2, sumX3),
            arrayOf(sumX, sumX2, sumX3, sumX4),
            arrayOf(sumX2, sumX3, sumX4, sumX5),
            arrayOf(sumX3, sumX4, sumX5, sumX6),
        )
    }
    ...
    override fun buildVector(points: List<PointDto>): Array<BigDecimal> {
        ...
        val sumX3Y = points.sumOf { it.x * it.x * it.x * it.y}
    ...
    override fun buildFormula(coefficients: List<BigDecimal>): String {
        ...
        val a3 = coefficients[3].round()

        return "y = ${a3}x³ + ${a2}x² + ${a1}x + ${a0}"
    }
}